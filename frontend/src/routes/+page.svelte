<script lang="ts">
    ...
    let encodedDraft = $state(
        registerDocument.registers[0]
            ? formatRegisterValue(
                  registerDocument.registers[0],
                  resetValuesById.get(registerDocument.registers[0].id) || 0n,
                  "enum",
              )
            : "",
    );
        ...
        >
            <div class="mx-auto flex max-w-6xl items-center gap-2">
                ...
                <Dialog.Root bind:open={settingsOpen}>
                    ...
                    >
                        <Dialog.Header>
                            ...
                            <Dialog.Description
                                >Customize the register documentation view.</Dialog.Description
                            >
                        </Dialog.Header>
                            ...
                            >
                                <div>
                                    ...
                                    <p class="text-xs text-muted-foreground">
                                        Choose the interface color mode.
                                    </p>
                                </div>
                                <Select.Root
                                    type="single"
                ...
                <Dialog.Root bind:open={aboutOpen}>
                    ...
                    >
                        <Dialog.Header>
                            ...
                            <Dialog.Description
                                >Build information embedded in this document.</Dialog.Description
                            >
                        </Dialog.Header>
                        <dl
                            class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-2 py-2 text-xs"
                        >
                        ...
                        {#if !registerDocument.metadata.length}
                            <p class="text-xs text-muted-foreground">
                                No build metadata was embedded.
                            </p>
                        {/if}
                    </Dialog.Content>
                </Dialog.Root>
                ...
                <Dialog.Root bind:open={shortcutsOpen}>
                    <Dialog.Content
                        class="border bg-card text-card-foreground shadow-xl ring-0 sm:max-w-md"
                    >
                        <Dialog.Header>
...
{#snippet fieldCard(field: RegisterField)}
    <Card
        id={`field-${encodeURIComponent(field.id)}`}
